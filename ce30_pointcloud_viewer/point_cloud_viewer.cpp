...
/** 定时器处理函数(对应的startTimer())
  *              1-初次启动，创建socket连接，创建数据接收线程，创建PCLVisualizer
  *              2-等待接收到的数据帧，更新显示
  * @param event:
  * @return
  */
...
/** 更新点云显示
  * @param scan: 一帧点云数据
  * @param viz: PCLVisualizer
  * @param vsmode: true-垂直拉伸模式
...
void PointCloudViewer::OnPCVizInitialized()
{
    ...
    pcviz_->AddCtrlShortcut(
    {
        ...
        [this]()
        {
            vertical_stretch_mode_ = !vertical_stretch_mode_;
        },
